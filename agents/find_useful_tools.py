...
find_useful_tools_chat_prompt_template = ChatPromptTemplate([
    ...
    ("system", "You are expert in finding functions and tools that can reduce AI hallucinations"),
    ("system", "For each suggested tool and function, "
...
tool_category_descriptions = {
    ...
    ToolCategory.EXTERNAL_TOOL_CATEGORY:
        "a tool that retrieval data from an external source (e.g., databases, APIs, search engines).",
    ToolCategory.STRUCTURED_TOOL_CATEGORY:
        "a tool that performs structured operations (e.g., math, date calculations, unit conversions).",
}
...
def find_useful_tools(state: State):
    ...
    for suggested_tool in response.suggested_tools:
        suggested_tool.candidate_for_generation = tool_category_candidate_for_generation[suggested_tool.category]
    ...
    return {"useful_tools": useful_tools,
