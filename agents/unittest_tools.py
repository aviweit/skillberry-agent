...
unittest_function_chat_prompt_template = ChatPromptTemplate.from_messages([
    ...
    ("system", "Additional use-cases should include boundary testing "),
    ("system", "Additional use-cases should include code coverage testing "),
...
def parse_str(result):
    if result is None or result == "" or result == "None" or result == "null" or result == json.dumps(None):
        return None
    try:
...
def compare_output(actual, expected) -> bool:
    actual = parse_str(actual)
    expected = parse_str(expected)
...
def validate_tool_using_llm_as_a_coder(name: str, description: str, metadata: dict, code: str) -> str:
    ...
    try:
        ...
        for unittest in unittests:
            ...
            if not compare_output(return_value, expected_value):
